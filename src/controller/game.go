...
func Game() {
	...
	for terminal.Scan() {
		...
		case "2":
			fmt.Println("Online play is not yet implemented")
		case "3":
			board := setupSinglePlayer(terminal)
...
func setupSinglePlayer(terminal bufio.Scanner) *model.Board {
	/*
	...
	*/
	...
	if confirmation != "1" {
		return setupSinglePlayer(terminal)
	}
